...
async function makeNote(username, domain, guid, params){
    const { published, name, content, sensitive, to, cc, url, n_attachs, href, mediaType, blurhash, width, height, summary, inReplyTo, pub, followshare, tags } = params;
    console.log("makeNote", guid, url_link)
    var url_link;
    if(!url){
    ...
    if(href && href != null){
        ...
        for(let i = 0; i < n_attachs; i++){
            if(urls[i] !== undefined){
                ...
                a.url = urls[i];
                a.name = "Untitled"
                if(blurhash===undefined || (blurhash[i] === undefined || blurhash[i] == null)){
