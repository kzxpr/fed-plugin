const { addName, addContent, addSummary, addAttachments, addTags } = require("./../utils/autofields");
const { makeNote, makeImage, makeEvent, makeQuestion } = require("./makeMessage");

async function makeObject(object, params, body){
    const { domain, username, guid, published, activity } = params;
    // NOTE: activity (type) is just used to handle autofillings
    ...
    var to = process.env.DEFAULT_TO_URI || "";
    
    if(body.to !== undefined){
        to = body.to
    }else if((activity == "Follow") && (body.stringobj !== undefined)){
        to = body.stringobj;
    }
    ...
    if(body.href !== undefined){
        if(Array.isArray(body.href)){
            // If body.href is already an array,
            // simply copy it
            href = body.href;
            ...
            // If body.href is NOT array (assuming string),
            // wrap it in one
            href = new Array(body.href)
            mediaType = new Array(body.mediaType)
            blurhash = new Array(body.blurhash)
            width = new Array(body.width);
            height = new Array(body.height)
            attachname = new Array(body.attachname)
        }
    }
    ...
    /*var n_tags = 0;
    if(body.n_tags !== undefined){
        n_tags = body.n_tags;
    }else if(tags){
        n_tags = tags.length; // Don't remember why I do this...?
    }*/
    ...
    if(object=="Id"){
        body += "<tr><td>string:</td><td><input type='text' name='stringobj' value='"+stringobj+"' style='width: 100%; max-width: 300px;'> (url)</td><td>(don't use '@' syntax - e.g. 'https://instance.com/@username/123456789' is 'https://instance.com/users/username/statuses/123456789')</td></tr>";
        hidden += "<input type='hidden' name='stringobj' value='"+stringobj+"'>";
        obj = stringobj;
    }else if(object=="Note"){
        obj = await makeNote(username, domain, manual_guid, { published, name, n_attachs, href, mediaType, tags, content, to, cc, sensitive, url, summary, inReplyTo, pub, followshare, updated, blurhash, width, height, attachname })

        //console.log("names", obj.attachment)
        // OVERWRITE
        if(obj.attachment && obj.attachment.length>0){
            blurhash = obj.attachment.map(v => v.blurhash)
            //attachname = obj.attachment.map(v => v.attachname)
            width = obj.attachment.map(v => v.width)
            height = obj.attachment.map(v => v.height)
        }
        
        const content_field = addContent({content})
        const summary_field = addSummary({summary})
        const attachment_field = addAttachments({ mediaType, href, n_attachs, blurhash, width, height, attachname });
        const tags_field = addTags({ tags, n_tags })

        body += summary_field.body + content_field.body + attachment_field.body + tags_field.body;
        hidden += summary_field.hidden + content_field.hidden + attachment_field.hidden + tags_field.hidden;
        ...
        obj = await makeQuestion(username, domain, manual_guid, { published, content, tags, to, cc, sensitive, questiontype, options, endTime, closed, pub, followshare, n_attachs, mediaType, href, url })
